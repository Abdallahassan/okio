...
public final class Utf8 {
  ...
  public static long size(String string) {
    Coverage.visitedge("Utf8::size", "start2");
  ...
  public static long size(String string, int beginIndex, int endIndex) {
    Coverage.visitedge("Utf8::size", "start");
    if (string == null) {
      Coverage.visitedge("Utf8::size", "string=null");
      throw new IllegalArgumentException("string == null");
    }
    if (beginIndex < 0) {
      Coverage.visitedge("Utf8::size", "beginindex<0");
    ...
    if (endIndex > string.length()) {
      Coverage.visitedge("Utf8::size", "endindex>length");
    ...
    for (int i = beginIndex; i < endIndex;) {
      ...
      if (c < 0x80) {
        Coverage.visitedge("Utf8::size", "c<0x80");
        // A 7-bit character with 1 byte.
        result++;
        i++;

      } else if (c < 0x800) {
        Coverage.visitedge("Utf8::size", "c<0x800");
        // An 11-bit character with 2 bytes.
        result += 2;
        i++;

      } else if (c < 0xd800 || c > 0xdfff) {
        Coverage.visitedge("Utf8::size", "c<0xd800|c>0xdfff");
        // A 16-bit character with 3 bytes.
        result += 3;
        i++;

      } else {
        Coverage.visitedge("Utf8::size", "else1");
        int low = i + 1 < endIndex ? string.charAt(i + 1) : 0;
        if (c > 0xdbff || low < 0xdc00 || low > 0xdfff) {
          Coverage.visitedge("Utf8::size", "c>0xdbff|low<0xdc00|low>0xdfff");
          // A malformed surrogate, which yields '?'.
          result++;
          i++;

        } else {
          Coverage.visitedge("Utf8::size", "else2");
    ...
    Coverage.visitedge("Utf8::size", "return");
    return result;
  }
}
